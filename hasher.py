		...
		while len(listaDirectorios) != 0:
			...
			while len(listaFicheros) != 0:
				...
				if not os.path.isdir(candidato):
					...
					h = HashingData(now, candidato, hasheo)
					mapaInicial[candidato] = hasheo
					ficheroSalida.write(str(h))
...
def comparacion():
		...
		while len(listaDirectorios) != 0:
			...
			while len(listaFicheros) != 0:
				...
				if not os.path.isdir(candidato):
					...
					hasheo = hasher(candidato, c.getMetodo())
					h = HashingData(now, candidato, hasheo)
					#ficheroSalida.write(hasher(candidato) + ": " + candidato + "\n")
					#print str(h)
	...
	for key in mapaComparado:
		lista.append(key)
		if mapaInicialTemporal.has_key(key):
			hash1 = mapaComparado[key]
			hash2 = mapaInicialTemporal[key]
			if hash1 != hash2:
				ficheroErrores.write("El fichero " + key.encode("utf-8") + " no mantiene su integridad\n")
				cambios += 1
		else:
			ficheroErrores.write("El fichero " + key.encode("utf-8") + " es nuevo\n")
			cambios += 1
			keyNueva = str(key)
	...
	for x in lista:
		if mapaInicialTemporal.has_key(x):
			del mapaInicialTemporal[x]
		del mapaComparado[x]

	if mapaInicialTemporal:
		for key in mapaInicialTemporal:
			ficheroErrores.write("El fichero " + key.encode("utf-8") + " se ha eliminado\n")
			cambios += 1

	mapaInicial.update(mapaTemporal)
	print "Cambios = " + str(cambios)
